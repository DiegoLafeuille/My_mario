...
class Overworld:
    ...
    def get_move_data(self, target):
        start = pygame.math.Vector2(levels[self.current_level]['node_pos'])
        if target == 'next': end = pygame.math.Vector2(levels[self.current_level + 1]['node_pos'])
        else:  end = pygame.math.Vector2(levels[self.current_level - 1]['node_pos'])
